...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
        ...
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav mr-auto">
                <a class="nav-item nav-link active" href="/activity/user/list">My Activities <span class="sr-only">(current)</span></a>
    ...
    <br/><h2 class="text-center">My Activities</h2><br/>

    <div class="container">

        <div class="text-right">
            <a type="button" class="btn btn-primary" href="/activity/user/add">Add Activity</a>
        </div>
        <br />
        <table class="table table-striped table-bordered">
            ...
            <c:forEach items="${activities}" var="activity">
                <c:url var="updateLink" value="/activity/user/update">
                    <c:param name="activityId" value="${activity.id}" />
                </c:url>

                <c:url var="deleteLink" value="/activity/user/delete">
                    <c:param name="activityId" value="${activity.id}" />
                </c:url>
        ...
        <nav aria-label="Navigation for activities">
            <ul class="pagination justify-content-end">
                <c:if test="${currentPage != 1}">
                    <li class="page-item">
                        <a class="page-link" href="/activity/user/list?currentPage=${currentPage-1}">Previous</a>
                    </li>
                </c:if>
                ...
                <c:forEach begin="1" end="${pages}" var="i">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <li class="page-item">
                                <a class="page-link" href="/activity/user/list?currentPage=${i}">${i}</a>
                            </li>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                ...
                <c:if test="${currentPage lt pages}">
                    <li class="page-item">
                        <a class="page-link" href="/activity/user/list?currentPage=${currentPage+1}">Next</a>
                    </li>
                </c:if>
            </ul>
        </nav>
